...
new class extends Component {
    ...
    public function mount(): void
    {
        ...
        } elseif ($user->hasRole('employee-manager')) {
            $this->tipo = 'Empleado';
        }
    }
    ...
    {
        $this->authorize('import', [\App\Models\RH\Collaborator::class, $this->tipo]);
        ...
        $path = $this->archivo->store('imports/colaboradores/' . auth()->id(), 'local');
...
<div class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        ...
        <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">Seleccione el tipo de colaboradores a importar y descargue la plantilla.</p>

        <div class="space-y-5">

            {{-- Tipo de colaborador --}}
            @if(auth()->user()->hasAnyRole(['super-admin', 'admin', 'rh-manager']))
            <div>
                <p class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Tipo de colaborador <span class="text-red-500" aria-hidden="true">*</span></p>
                <div class="flex gap-3" role="radiogroup" aria-label="Tipo de colaborador">
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border px-4 py-2.5 text-sm transition-colors
                        {{ $tipo === 'Empleado' ? 'border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300' }}">
                        <input type="radio" wire:model.live="tipo" value="Empleado" class="sr-only" />
                    ...
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg border px-4 py-2.5 text-sm transition-colors
                        {{ $tipo === 'Contratista' ? 'border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300' }}">
                        <input type="radio" wire:model.live="tipo" value="Contratista" class="sr-only" />
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
            ...
            @else
            <div class="rounded-lg bg-blue-50 px-4 py-3 text-sm text-blue-700 dark:bg-blue-900/20 dark:text-blue-300" role="note">
                Solo puede importar <strong>{{ $tipo === 'Empleado' ? 'Empleados' : 'Contratistas' }}</strong> según su rol.
            </div>
            @endif
            ...
            <div>
                <label class="flex cursor-pointer items-start gap-3 rounded-lg border border-gray-200 p-4 hover:border-gray-300 dark:border-gray-700 dark:hover:border-gray-600">
                    <input type="checkbox" wire:model.live="sobrescribir" class="mt-0.5 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-600" />
                    <div>
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Actualizar colaboradores existentes</p>
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                            Si un colaborador con el mismo documento ya existe, se actualizarán sus datos.
            ...
            <div class="rounded-xl border border-dashed border-blue-200 bg-blue-50/50 p-5 dark:border-blue-800/50 dark:bg-blue-900/10">
                <p class="mb-3 text-sm font-medium text-gray-700 dark:text-gray-300">
                    Descargue la plantilla oficial para registrar los colaboradores:
                </p>
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('rh.colaboradores.plantilla', $tipo === 'Empleado' ? 'empleados' : 'contratistas') }}"
                       class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900">
                        ...
                        Descargar plantilla {{ $tipo === 'Empleado' ? 'Empleados' : 'Contratistas' }} (.xlsx)
                    </a>
                    @if(auth()->user()->hasAnyRole(['super-admin', 'admin', 'rh-manager']))
                    <a href="{{ route('rh.colaboradores.plantilla', 'todos') }}"
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div class="mb-4 flex flex-wrap gap-3">
            <span class="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                {{ $tipo === 'Empleado' ? 'Empleados' : 'Contratistas' }}
            </span>
            @if($sobrescribir)
