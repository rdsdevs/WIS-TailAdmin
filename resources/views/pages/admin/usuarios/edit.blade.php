...
@section('content')
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Editar usuario</h2>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                Actualice los datos del usuario <strong class="text-gray-700 dark:text-gray-300">{{ $usuario->name }}</strong>.
            </p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
        <form method="POST" action="{{ route('admin.usuarios.update', $usuario) }}" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select
                            ...
                            @foreach ($instituciones as $inst)
                                <option value="{{ $inst->id }}" @selected(old('institution_id', $usuario->institution_id) === $inst->id)>
                                    {{ $inst->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <select
                        ...
                        @foreach (['CC' => 'Cédula de Ciudadanía', 'CE' => 'Cédula de Extranjería', 'NIT' => 'NIT', 'PP' => 'Pasaporte', 'TI' => 'Tarjeta de Identidad'] as $val => $label)
                            <option value="{{ $val }}" @selected(old('document_type', $usuario->document_type) === $val)>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('document_type')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('document_number', $usuario->document_number) }}"
                        inputmode="numeric"
                        class="w-full rounded-lg border px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 dark:bg-gray-800 dark:text-white
