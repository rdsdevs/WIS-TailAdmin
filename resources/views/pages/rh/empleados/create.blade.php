...
@section('content')
    ...
    {{-- Formulario --}}
    <form method="POST" action="{{ route('rh.empleados.store') }}" novalidate>
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div
                x-data="{
                    departamento_id: '{{ old('department_id') }}',
                ...
                <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                    ...
                    <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                        Ubicación dentro de la organización y condiciones salariales.
                    </p>
                </div>
                <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-2">
                    ...
                    <div class="space-y-1.5">
                        ...
                        >
                            ...
                            <template x-for="cargo in cargos" :key="cargo.id">
                                <option :value="cargo.id" x-text="cargo.name"></option>
                            </template>
                            {{-- Fallback sin Alpine: mostrar todos cuando no hay departamento seleccionado --}}
                            <template x-if="cargos.length === 0">
                                @foreach($posiciones ?? [] as $posicion)
                                    <option value="{{ $posicion->id }}" {{ old('position_id') === $posicion->id ? 'selected' : '' }}>
                                        {{ $posicion->name }}
                                    </option>
                                @endforeach
                            </template>
