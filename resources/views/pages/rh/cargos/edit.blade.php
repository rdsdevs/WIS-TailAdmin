...
@section('content')
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                Modifique los datos del cargo <strong>{{ $cargo->name }}</strong>.
            </p>
        </div>
    ...
    <div class="mt-6">
        <form method="POST" action="{{ route('rh.cargos.update', $cargo) }}"
              x-data="{
                  emails: @json(old('emails', $cargo->emails->pluck('email')->toArray())),
                  functions: @json(old('functions', $cargo->functions->pluck('description')->toArray())),
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select id="department_id" name="department_id" required
                                    ...
                                    @foreach($departamentos as $dep)
                                        <option value="{{ $dep->id }}"
                                            {{ old('department_id', $cargo->department_id) === $dep->id ? 'selected' : '' }}>
                                            {{ $dep->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                          class="wis-input w-full {{ $errors->has('description') ? 'border-red-400' : '' }}">{{ old('description', $cargo->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="sm:col-span-2 flex items-center gap-3">
                                <label class="relative inline-flex cursor-pointer items-center">
                                    ...
                                    <span class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">Cargo activo</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <div class="space-y-3">
                            <template x-if="functions.length === 0">
                                <p class="text-sm text-gray-400 dark:text-gray-500 italic">No hay funciones registradas.</p>
                            </template>
